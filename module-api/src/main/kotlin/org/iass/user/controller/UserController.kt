...
class UserController(
	...
	fun login(
		...
		ResponseEntity.ok(ApiResponse.success(SuccessType.POST_APPLE_LOGIN_SUCCESS, userFacade.login(authorization, request)))
	...
	fun reissue(
		...
		ResponseEntity.ok(ApiResponse.success(SuccessType.POST_REISSUE_SUCCESS, userFacade.reissue(authorization)))

	@PostMapping("/logout")
	fun logout(principal: Principal): ResponseEntity<ApiResponse<Nothing>> {
	...
	fun withdrawal(principal: Principal): ResponseEntity<ApiResponse<Nothing>> {
